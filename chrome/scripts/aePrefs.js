...
let aePrefs = {
  ...
  {
    let pref = await chrome.storage.local.get(aPrefName);
  ...
  async getAllPrefs()
  {
    let rv = await chrome.storage.local.get(this.getPrefKeys());
    return rv;
  },

  async setPrefs(aPrefMap)
  {
    await chrome.storage.local.set(aPrefMap);
  },

  async setDefaultBkgdState()
  {
    await chrome.storage.local.set(this._defaultBkgdState);
  },
  ...
  async _removePrefs(aCurrPrefs, aOldPrefs)
  {
    ...
    await chrome.storage.local.remove(aOldPrefs);
  },
};
