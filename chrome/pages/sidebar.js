...
let gCmd = {
  ...
  async insertClipping()
  {
    ...
    let wnd = await chrome.windows.getCurrent({populate: true});
    let tabs = wnd.tabs;
    ...
    try {
      ...
      // selected clipping.
      resp = await chrome.tabs.sendMessage(actvTab.id, {msgID: "focus-active-tab"});
    }
    catch (e) {
  ...
  showHelp()
  {
    chrome.runtime.sendMessage({msgID: "open-sidebar-help"});
  },
  ...
  _openClippingsMgr(aMsgInfo)
  {
    ...
    chrome.runtime.sendMessage(aMsgInfo);
  },
};
...
$(async () => {
  ...
  $("#help").attr("title", chrome.i18n.getMessage("tbHelp"));

  // Show update message bar if Clippings was just updated.
  let {verUpdateType, showBanner} = await chrome.runtime.sendMessage({msgID: "get-ver-update-info"});
...
function initDialogs()
{
  ...
  gClipbdWritePermMsgBox.onFirstInit = function ()
  {
    ...
    $("#perm-request").text(chrome.i18n.getMessage("extPermInstr", extName));
  };
...
async function showVersionUpdateMsgBar(aVersionUpdateType)
{
  if (aVersionUpdateType == aeConst.VER_UPDATE_TYPE_MAJOR) {
    ...
    try {
      let resp = await chrome.runtime.sendMessage({msgID: "ping-whats-new-pg"});
  ...
  aeClippingsTree.build(rootFldrID, gPrefs).then(aTreeData => {
	...
	case "reloadSyncFolder":
	  ...
	  gCmd.openWebPageSourceURL();
        ...
        default:
          alert(chrome.i18n.getMessage("msgUnavail"));
          break;
        }
      },
      ...
      items: {
        ...
        insertClipping: {
          name: chrome.i18n.getMessage("insClipping"),
          className: "ae-menuitem",
          visible(aItemKey, aOpt) {
	...
	togglePreviewPane: {
	  name: chrome.i18n.getMessage("mnuPrevPane"),
	  className: "ae-menuitem",
	...
	customize: {
	  name: chrome.i18n.getMessage("mnuCustz"),
	  className: "ae-menuitem",
	}
      }
    });
