...
$(async () => {
  ...
  if (gOS == "win") {
    let prefPgTitleWin = chrome.i18n.getMessage("prefsTitleWin");
  ...
  let hostApp = chrome.i18n.getMessage("hostAppFx");
  $("#ext-perm-native-msg").text(chrome.i18n.getMessage("extPrmNativeMessaging", hostApp));
  $("#ext-perm-native-msg-detail").html(
    sanitizeHTML(chrome.i18n.getMessage("syncPermReqDetail", aeConst.SYNC_CLIPPINGS_DWNLD_URL))
  );
  ...
  usrContribCTA.append(sanitizeHTML(`<label id="usr-contrib-cta-hdg">${chrome.i18n.getMessage("aboutContribHdg")}</label>&nbsp;`));

  let contribLinks = chrome.i18n.getMessage("aboutContrib", [aeConst.DONATE_URL, aeConst.L10N_URL]);
  usrContribCTA.append(sanitizeHTML(contribLinks));

  // Sync Clippings help dialog content.
  let syncHlpTxt = chrome.i18n.getMessage("syncHelp", aeConst.SYNC_CLIPPINGS_HELP_URL);
  ...
  $("#backup-reminder").prop("checked", (prefs.backupRemFrequency != aeConst.BACKUP_REMIND_NEVER)).on("click", async (aEvent) => {
    ...
    await chrome.runtime.sendMessage({msgID: "clear-backup-notifcn-intv"});
    if (aEvent.target.checked) {
      chrome.runtime.sendMessage({msgID: "set-backup-notifcn-intv"});
    }
  });
...
function initDialogs()
{
  ...
  gDialogs.reqNativeAppConxnPerm.onAccept = async function ()
  {
    ...
    let permGranted = await chrome.permissions.request({
  ...
  {
    $("#no-sync-app-cta").html(sanitizeHTML(chrome.i18n.getMessage("noSyncAppCTA", aeConst.SYNC_CLIPPINGS_DWNLD_URL)));
  ...
  gDialogs.syncClippings.onInit = async function ()
  {
    ...
    let resp;
    try {
      resp = await chrome.runtime.sendNativeMessage(aeConst.SYNC_CLIPPINGS_APP_NAME, {msgID: "get-app-version"});
    }
    catch (e) {
    ...
    if (aeVersionCmp(resp.appVersion, "2.0b1") < 0) {
      ...
      $("#cmprs-sync-data-reqmt").html(
        chrome.i18n.getMessage("cmprsSyncReqmt", aeConst.SYNC_CLIPPINGS_DWNLD_URL)
      ).show();
    ...
    resp = await chrome.runtime.sendNativeMessage(aeConst.SYNC_CLIPPINGS_APP_NAME, {msgID: "get-sync-dir"});
  ...
  gDialogs.syncClippings.onAccept = async function ()
  {
    ...
    let syncFolderID = await chrome.runtime.sendMessage({
  ...
  gDialogs.turnOffSync.onFirstInit = function ()
  {
    this.find(".dlg-btns > .dlg-btn-yes").on("click", async (aEvent) => {
      ...
      let oldSyncFolderID = await chrome.runtime.sendMessage(msg);
  ...
  gDialogs.wndsDlgsOpts.onFirstInit = function ()
  {
    if (! ["win", "mac"].includes(gOS)) {
      let os = gOS == "mac" ? chrome.i18n.getMessage("macOS") : capitalize(gOS);
      $("#wnds-dlgs-opts-exp-warn-msg").text(chrome.i18n.getMessage("wndsDlgsOptsExpWarn", os));
  ...
  gDialogs.wndsDlgsOpts.onAccept = async function (aEvent)
  {
    ...
    try {
      isClippingsMgrOpen = await chrome.runtime.sendMessage({msgID: "ping-clippings-mgr"});
    }
    catch (e) {}
  ...
  gDialogs.about.onInit = function ()
  {
    ...
    this.find("#ext-ver").text(chrome.i18n.getMessage("aboutExtVer", this.extInfo.version));
  ...
  gDialogs.about.onShow = async function ()
  {
    ...
    try {
      ...
      this.find("#sync-ver").text(chrome.i18n.getMessage("noSyncHelperApp"));
    }
    ...
    if (syncClippings) {
      ...
      this.find("#about-sync-status").text(chrome.i18n.getMessage("syncStatusOff")).show();
    }
