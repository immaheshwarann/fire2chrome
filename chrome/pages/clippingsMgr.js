...
let gSrcURLBar = {
  init: function ()
  {
    ...
    $("#edit-src-url-ok").attr("title", chrome.i18n.getMessage("btnOK")).on("click", aEvent => { this.acceptEdit() });
    $("#edit-src-url-cancel").attr("title", chrome.i18n.getMessage("btnCancel")).on("click", aEvent => { this.cancelEdit() });
  },
  ...
  acceptEdit: function ()
  {
    ...
    gClippingsSvc.updateClipping(clippingID, {
      ...
      if (updatedURL && gSyncedItemsIDs.has(clippingID + "C")) {
        chrome.runtime.sendMessage({msgID: "push-sync-fldr-updates"})
...
let gCmd = {
  ...
  async moveClippingOrFolder()
  {
    ...
    if (selectedNode) {
      if (selectedNode.isFolder()) {
        ...
        let pingResp;
        try {
          pingResp = await chrome.runtime.sendMessage({msgID: "ping-new-clipping-dlg"});
        }
        catch {}
        if (pingResp) {
  ...
  async deleteClippingOrFolder(aDestUndoStack)
  {
    ...
    if (selectedNode.isFolder()) {
      ...
      gClippingsDB.folders.get(id).then(aFolder => {
        ...
        if (gSyncedItemsIDs.has(parentFolderID + "F")) {
          chrome.runtime.sendMessage({msgID: "push-sync-fldr-updates"}).then(() => {
  ...
  async insertSeparator(aDestUndoStack)
  {
    ...
    let newSeparator = {
      name: chrome.i18n.getMessage("sepName"),
      content: "",
      shortcutKey: "",
      parentFolderID,
      label: "",
    ...
    if (gSyncedItemsIDs.has(parentFolderID + "F")) {
      ...
      let resp;
      try {
        resp = await chrome.runtime.sendMessage({msgID: "push-sync-fldr-updates"});
    ...
    return new Promise((aFnResolve, aFnReject) => {
      ...
      gClippingsDB.clippings.get(aClippingID).then(aClipping => {
        ...
        if (gSyncedItemsIDs.has(aNewParentFldrID + "F")
          ...
          chrome.runtime.sendMessage({msgID: "push-sync-fldr-updates"}).then(aResp => {
  ...
  copyFolderIntrl(aFolderID, aDestFldrID, aDestUndoStack)
  {
    ...
    chrome.runtime.sendMessage({msgID: "copy-started"});
  ...
  updateDisplayOrder: function (aFolderID, aDestUndoStack, aUndoInfo, aSuppressClippingsMenuRebuild)
  {
	...
	Promise.all(seqUpdates).then(aNumUpd => {
          ...
          if (! aSuppressClippingsMenuRebuild) {
            chrome.runtime.sendMessage({msgID: "rebuild-cxt-menu"});
          }
  ...
  insertClippingInClippingPlaceholder()
  {
    ...
    let arg = chrome.i18n.getMessage("plchldrClipClipArg");
  ...
  backup: function ()
  {
    ...
    aeImportExport.exportToJSON(INCLUDE_SRC_URLS, false, aeConst.ROOT_FOLDER_ID, excludeSyncFldrID, true, true).then(aJSONData => {
      ...
      chrome.downloads.download({
        ...
        return chrome.downloads.search({ id: aDownldItemID });
...
$(async () => {
  ...
  let brwsStub = {name: "Chrome", version: ""};
  let platform = await chrome.runtime.getPlatformInfo();
  gEnvInfo = {
    os: platform.os,
    hostAppName: brwsStub.name,
    hostAppVer:  brwsStub.version,
  };
  ...
  if (gEnvInfo.os == "mac") {
    ...
    $("#minz-when-inactv-mode").attr("data-checked", !!gPrefs.clippingsMgrMinzWhenInactv)
      .attr("title", chrome.i18n.getMessage("mnuMinimizeWhenInactive"));
  }
  ...
  // chrome.windows.create won't show contents unless resized.
...
function initToolbar()
{
  ...
  $("#move").attr("title", chrome.i18n.getMessage("tbMoveOrCopy")).on("click", aEvent => {
    gCmd.moveClippingOrFolder();
  });
  $("#delete").attr("title", chrome.i18n.getMessage("tbDelete")).on("click", aEvent => {
    gCmd.deleteClippingOrFolder(gCmd.UNDO_STACK);
  });
  $("#undo").attr("title", chrome.i18n.getMessage("tbUndo")).on("click", aEvent => {
    gCmd.undo();
  });
  $("#help").attr("title", chrome.i18n.getMessage("tbHelp")).on("click", aEvent => {
    gCmd.showMiniHelp();
  });
  ...
  $.contextMenu({
    ...
    items: {
      ...
      insClippingInClipping: {
        name: chrome.i18n.getMessage("mnuPlchldrClipClip"),
        className: "ae-menuitem"
      },
    }
  });
  ...
  $.contextMenu({
    ...
    items: {
      ...
      moreToolsSubmenu: {
        name: chrome.i18n.getMessage("mnuMoreTools"),
        items: {
          removeAllSrcURLs: {
...
function initIntroBannerAndHelpDlg()
{
  ...
  function buildKeyMapTable(aTableDOMElt)
  {
    ...
    if (isMacOS) {
      ...
      let altRedo;
      if (isWin) {
        altRedo = `${chrome.i18n.getMessage("keyCtrl")}+Y`;
      }
      else {
        altRedo = `${chrome.i18n.getMessage("keyCtrl")}+${chrome.i18n.getMessage("keyShift")}+Z`;
      }
      shctKeys = [
        chrome.i18n.getMessage("keyDel"),
        chrome.i18n.getMessage("keyEsc"),
        `${chrome.i18n.getMessage("keyCtrl")}+C`,
        `${chrome.i18n.getMessage("keyCtrl")}+D`,
        `${chrome.i18n.getMessage("keyCtrl")}+F`,
        `${chrome.i18n.getMessage("keyCtrl")}+W`,
        `${chrome.i18n.getMessage("keyCtrl")}+Z`,
        "F1",
        `F2 / ${altRedo}`,
        `${chrome.i18n.getMessage("keyCtrl")}+F10`,
      ];
    }
...
function initDialogs()
{
  ...
  gDialogs.requestExtPerm.onFirstInit = function ()
  {
    ...
    this.find("#grant-ext-perm").text(chrome.i18n.getMessage("extPermInstr", extName));
  };
  gDialogs.requestExtPerm.onInit = function ()
  {
    ...
    this.find(".dlg-content ul > li").text(chrome.i18n.getMessage(strKey));
  };
  ...
  {
    let keybPasteKeys = await chrome.runtime.sendMessage({msgID: "get-shct-key-prefix-ui-str"});
    let shctPrefixKey = 0;
    $("#shortcut-instrxns").text(chrome.i18n.getMessage("clipMgrShortcutHelpInstrxn", keybPasteKeys));
    let extVer = chrome.runtime.getManifest().version;
    
    aeImportExport.setL10nStrings({
      shctTitle: chrome.i18n.getMessage("expHTMLTitle"),
      hostAppInfo: chrome.i18n.getMessage("expHTMLHostAppInfo", [extVer, gEnvInfo.hostAppName]),
    ...
    $("#export-shct-list").on("click", aEvent => {
      aeImportExport.getShortcutKeyListHTML(true).then(aHTMLData => {
        ...
        return chrome.downloads.download(downldOpts);
  ...
  gDialogs.importFromFile.onAccept = function (aEvent)
  {
    ...
    if (this.mode == this.IMP_REPLACE) {
      ...
      aeImportExport.exportToJSON(true, false, aeConst.ROOT_FOLDER_ID, excludeSyncFldrID, true).then(aJSONData => {
        ...
        return chrome.runtime.sendMessage({msgID: "import-started"});
      ...
      chrome.runtime.sendMessage({msgID: "import-started"}).then(() => {
        importFile(true);
      });
    }
  };
  ...
  gDialogs.exportToFile.setProps({
    ...
    inclSep: false,
    fmtDesc: [
      chrome.i18n.getMessage("expFmtClippings6Desc"), // Clippings
  ...
  {    
    if (this.fldrTree) {
      ...
      this.fldrTree = new aeFolderPicker(
        ...
        chrome.i18n.getMessage("rootFldrName"),
  ...
  gDialogs.moveTo.onAccept = function (aEvent)
  {
    ...
    if (gSyncedItemsIDs.has(destFolderID + "F") && gPrefs.isSyncReadOnly) {
      $("#move-error").text(chrome.i18n.getMessage("syncFldrRdOnly"));
      return;
    }
...
function buildClippingsTree()
{
  ...
  aeClippingsTree.build(aeConst.ROOT_FOLDER_ID, gPrefs).then(aTreeData => {
    ...
    $("#clippings-tree").fancytree({
      ...
      source: treeData,
      selectMode: 1,
      strings: { noData: chrome.i18n.getMessage("clipMgrNoItems") },
      ...
      dnd5: {
        ...
        async dragDrop(aNode, aData)
        {
          ...
          if (aData.otherNode) {           
            ...
            await chrome.runtime.sendMessage({msgID: "dnd-move-started"});
            ...
            await chrome.runtime.sendMessage({msgID: "dnd-move-finished"});
	...
	case "reloadSyncFolder":
      ...
      items: {
        ...
        copyClippingText: {
          name: chrome.i18n.getMessage("mnuCopyClipTxt"),
          className: "ae-menuitem",
          visible(aItemKey, aOpt) {
            return (!aeClippingsTree.isFolderSelected() && !aeClippingsTree.isSeparatorSelected());
          }
        ...
        insertSeparator: {
          name: chrome.i18n.getMessage("mnuInsSeparator"),
          className: "ae-menuitem",
          disabled(aKey, aOpt) {
            let tree = aeClippingsTree.getTree();
            let selectedNode = tree.activeNode;
...
{
  let rv = [
    {title: chrome.i18n.getMessage("clipMgrNoItems"), key: "0"}
  ];
...
function handlePushSyncItemsError(aError)
{
  if (!gErrorPushSyncItems) {
    ...
    errorMsgBox.onInit = function () {
      this.find(".dlg-content > .msgbox-error-msg").text(chrome.i18n.getMessage("syncPushFailed"));
    };
    errorMsgBox.showModal();
