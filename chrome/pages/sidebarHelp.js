...
async function init()
{
  ...
  let pasteShct = chrome.i18n.getMessage("keyCtrl") + "+V";
  if (platform.os == "mac") {
    pasteShct = chrome.i18n.getMessage("keyCommand") + "V";
  }

  let txt = document.createTextNode(chrome.i18n.getMessage("sbarHlpWhat", pasteShct));
...
function initKeyboardShortcutTable(aOSName)
{
  ...
  function buildKeyMapTable(aTableDOMElt)
  {
    ...
    if (isMacOS) {
      ...
      shctKeys = ["F1", `${chrome.i18n.getMessage("keyCtrl")}+C`];
    }
