...
$(async () => {
  ...
  $("#clipping-text").attr("placeholder", chrome.i18n.getMessage("clipMgrContentHint"));
  ...
  $("#clipping-name").blur(aEvent => {
    ...
    if (! name) {
      $("#clipping-name").val(chrome.i18n.getMessage("untitledClipping"));
    }
  });
  ...
  gNewFolderDlg.selectFolder = function (aFolderData)
  {
    if (gPrefs.syncClippings && gPrefs.isSyncReadOnly) {
      ...
      if (folderID == gPrefs.syncFolderID || gSyncedFldrIDs.has(folderID)) {
        ...
        alert(chrome.i18n.getMessage("syncFldrRdOnly"));
        return;
      }
    }
...
function initFolderPicker()
{
  function selectSyncedClippingsFldr()
  {
    $("#new-clipping-fldr-picker-menubtn").val(gPrefs.syncFolderID)
      .text(chrome.i18n.getMessage("syncFldrName"))
      .attr("syncfldr", "true");
  }
...
async function finishAcceptDlg(aNewClippingID, aNewClipping)
{
  ...
  if (gPrefs.syncClippings) {
    ...
    let isSyncDataSizeUnderMax = await chrome.runtime.sendMessage({
    ...
    let resp = await chrome.runtime.sendNativeMessage(aeConst.SYNC_CLIPPINGS_APP_NAME, natMsg);
