...
// Chrome service workers do not have localStorage.  Use an in-memory object
// as a lightweight replacement.  Values are re-initialized on each service
// worker activation.
const gLocalState = {
  _data: {},
...
let gSyncClippingsListener = {
  ...
  async onDeactivate(aOldSyncFolderID)
  {
    ...
    chrome.contextMenus.update(syncFldrMenuID, mnuIco);
  },
  ...
  async onReloadFinish()
  {
    ...
    try {
      await chrome.runtime.sendMessage({msgID: "sync-fldr-reload-finished"});
    }
    catch {}
  },
};
...
chrome.runtime.onInstalled.addListener(async (aInstall) => {
...
chrome.runtime.onStartup.addListener(async () => {
  ...
  gLocalState.setItem("isInitialized", '');
...
async function init(aPrefs)
{
  ...
  log(`Clippings/wx: Host app: ${brws.name}`);
  ...
  if (aPrefs.syncClippings) {
    if (isInitialized) {
      ...
      try {
        await chrome.runtime.sendMessage({msgID: "startup-sync-clippings"});
      }
      catch {}
  ...
  if (! backupNotifcnAlarm) {
    ...
    chrome.alarms.create("show-backup-notifcn", {
  ...
  gLocalState.setItem("browserAction", aPrefs.browserAction);
  ...
  gLocalState.setItem("isInitialized", "1");
...
async function isSyncedClippingsReadOnly()
{
  ...
  let resp;
  try {
    resp = await chrome.runtime.sendNativeMessage(aeConst.SYNC_CLIPPINGS_APP_NAME, {
...
async function refreshSyncedClippings(aRebuildClippingsMenu)
{
  ...
  let syncJSONData = "";
  try {
    resp = await chrome.runtime.sendNativeMessage(aeConst.SYNC_CLIPPINGS_APP_NAME, natMsg);
  }
  catch (e) {
...
async function getShortcutKeyPrefixStr()
{
  ...
  let [cmd] = await chrome.commands.getAll();
  ...
  if (keys.includes(keybPasteKey)) {
    if (keybPasteKey == "Delete" && isMacOS) {
      localizedKey = chrome.i18n.getMessage("keyMacDel");
    }
    else {
      localizedKey = chrome.i18n.getMessage(`key${keybPasteKey}`);
    }
  }
  else {
  ...
  for (let i = 0; i < modifiers.length; i++) {
    ...
    if (isMacOS) {
      if (modifier == "Alt") {
        localzMod = chrome.i18n.getMessage("keyOption");
      }
      else if (modifier == "Ctrl") {
        localzMod = chrome.i18n.getMessage("keyCommand");
      }
      else if (modifier == "Shift") {
        localzMod = chrome.i18n.getMessage("keyMacShift");
      }
      else {
        localzMod = chrome.i18n.getMessage(`key${modifier}`);
      }
    }
    else {
      localzMod = chrome.i18n.getMessage(`key${modifier}`);
...
async function buildContextMenu(aPlatformOS, aPrefs)
{
  ...
  chrome.contextMenus.create({
    ...
    title: chrome.i18n.getMessage(prefsMnuStrKey),
    contexts: ["action"],
  });
...
async function updateContextMenuForFolder(aUpdatedFolderID)
{
  ...
  if (menuItemID) {
    chrome.contextMenus.update(menuItemID, {title: sanitizeMenuTitle(folder.name)});
  }
}
...
async function buildAutoIncrementPlchldrResetMenu(aAutoIncrPlchldrs)
{
  ...
  aAutoIncrPlchldrs.forEach(async (aItem, aIndex, aArray) => {
    if (! autoIncrPlchldrsSet.has(aItem)) {
      ...
      if (! enabledResetMenu) {
        await chrome.contextMenus.update("ae-clippings-reset-autoincr-plchldrs", {
          enabled: true
        });
...
async function resetAutoIncrPlaceholder(aPlaceholder)
{
  ...
  await chrome.contextMenus.remove(`ae-clippings-reset-autoincr-${aPlaceholder}`);
...
async function showBackupNotification()
{
  ...
  if (diff.days >= numDays || prefs._forceShowFirstTimeBkupNotif) {
    if (prefs.backupRemFirstRun) {
      ...
      await chrome.notifications.create("backup-reminder-firstrun", {
        type: "basic",
...
async function clearBackupNotificationInterval()
{
  ...
  await chrome.alarms.clear("show-backup-notifcn");
}
...
async function showWhatsNewNotification()
{
  ...
  if (prefs.upgradeNotifCount <= 0) {
    let alarm = await chrome.alarms.get("show-upgrade-notifcn");
    if (!!alarm) {
      warn("Clippings: showWhatsNewNotification(): Alarm failed to clear.  Attempting to clear it again and resetting prefs.");

      let isCleared = await chrome.alarms.clear("show-upgrade-notifcn");
      if (! isCleared) {
  ...
  if (diff.days >= aeConst.POST_UPGRADE_NOTIFCN_FREQ_DAYS) {
    ...
    await chrome.notifications.create("whats-new", {
      type: "basic",
      title: extName,
      message: chrome.i18n.getMessage("upgradeNotifcn", extName),
      iconUrl: "img/notifIcon.svg",
    });
...
async function setSyncHelperUpdateNotificationDelay(aEnableNotifcn)
{
  ...
  if (aEnableNotifcn) {
    ...
    chrome.alarms.create("show-sync-helper-upd-notifcn", {
    ...
    await chrome.alarms.clear("show-sync-helper-upd-notifcn");
...
async function showSyncHelperUpdateNotification()
{
  ...
  if (!prefs.lastSyncHelperUpdChkDate || diff.days >= aeConst.SYNC_HELPER_CHECK_UPDATE_FREQ_DAYS) {
    ...
    if (aeVersionCmp(currVer, updateInfo.latestVersion) < 0) {
      ...
      chrome.notifications.create("sync-helper-update", {
        type: "basic",
...
{
  chrome.notifications.create("sync-data-size-too-big", {
    type: "basic",
    title: chrome.i18n.getMessage("syncClippings"),
    message: chrome.i18n.getMessage("syncFldrFull"),
...
{
  let url = chrome.runtime.getURL("pages/welcome.html");
...
async function openClippingsManager(aBackupMode)
{
  ...
  async function openClippingsMgrHelper()
  {
    ...
    let [tab] = await chrome.tabs.query({currentWindow: true, discarded: false});
    ...
    let wnd = await chrome.windows.create(wndInfo);
    ...
    // `chrome.windows.create()`. If unable to get window geometry, then
    // default to centering on screen.
    if (wndGeom) {
      chrome.windows.update(wnd.id, {left, top});
    }
  }
  // END nested function
  ...
  if (wndIDs.clippingsMgr) {
    try {
      let wnd = await chrome.windows.get(wndIDs.clippingsMgr);
      await chrome.windows.update(wndIDs.clippingsMgr, {focused: true});

      if (aBackupMode) {
        try {
          chrome.runtime.sendMessage({msgID: "clippings-mgr-save-backup"});
        }
        catch {}
      }
    }
...
async function newClipping(aActiveTab)
{
  ...
  let resp;
  try {
    resp = await chrome.tabs.sendMessage(aActiveTab.id, {msgID: "new-clipping"});
  }
  catch (e) {
...
function openKeyboardPasteDlg(aTabID, aIsExpanded)
{
  ...
  let url = chrome.runtime.getURL("pages/keyboardPaste.html?tabID=" + aTabID);
  let wndPpty = {
    type: "popup",
    width: 500,
    height: 164,
...
function openPlaceholderPromptDlg(aTabID, aDlgMode, aIsExpanded)
{
  ...
  let url = chrome.runtime.getURL(`pages/placeholderPrompt.html?tabID=${aTabID}&mode=${aDlgMode}`);
  let wndPpty = {
    type: "popup",
    width: 536,
    height: 228,
...
{
  let url = chrome.runtime.getURL(`pages/pasteAs.html?tabID=${aTabID}`);
  let wndPpty = {
    type: "popup",
    width: 440,
    height: 300,
...
{
  let url = chrome.runtime.getURL("pages/backup.html");
...
{
  let url = chrome.runtime.getURL("pages/sidebarHelp.html");
  let height = 406;
  let {os} = await chrome.runtime.getPlatformInfo();
...
async function openDlgWnd(aURL, aWndKey, aWndPpty, aTabID=null, aAlwaysCalcWndPosFromBrwsWnd=false, aIsMacOSFullScreen=false)
{
  ...
  if (aAlwaysCalcWndPosFromBrwsWnd) {
    ...
    let wnds = await chrome.windows.getAll({windowTypes: ["normal"]});
  ...
  if (wndIDs[aWndKey]) {
    try {
      await chrome.windows.get(wndIDs[aWndKey]);
      chrome.windows.update(wndIDs[aWndKey], {focused: true});
    }
    catch (e) {
      wndIDs[aWndKey] = null;
      aePrefs.setPrefs({_wndIDs: wndIDs});
...
{
  let rv = null;
  let tab = await chrome.tabs.get(aTabID);
  let wnd = await chrome.windows.get(tab.windowId);
...
async function processClipping(aClippingInfo, aIsExternalRequest, aTabID, aMode)
{
  ...
  if (aeClippingSubst.hasNoSubstFlag(aClippingInfo.name)) {
    ...
    if (plchldrs.length > 0) {
      ...
      let platform = await chrome.runtime.getPlatformInfo();
      if (platform.os == "mac") {
        let currWnd = await chrome.windows.getCurrent();
...
async function processHTMLFormattedClipping(aClippingName, aClippingContent, aTabID)
{
  ...
  let tab;
  try {
    tab = await chrome.tabs.get(aTabID);
  }
  catch (e) {
  ...
  let isHTMLEditor = await chrome.tabs.sendMessage(aTabID, {msgID: "is-html-editor?"});
  if (isHTMLFormatted) {
...
async function pasteProcessedClipping(aClippingContent, aTabID, aOverridePasteFormat=null)
{
  ...
  setTimeout(async () => {
    try {
      await chrome.tabs.sendMessage(aTabID, msg);
    }
    catch (e) {
...
async function openSyncClippingsDownloadPage()
{
  ...
  chrome.tabs.create({url: syncClippingsHelperDwnldPgURL});
}
...
{
  chrome.notifications.create("sync-read-error", {
    type: "basic",
    title: chrome.i18n.getMessage("syncStartupFailedHdg"),
    message: chrome.i18n.getMessage("syncGetFailed"),
    iconUrl: aeVisual.getErrorIconPath(),
  });
}
...
{
  chrome.notifications.create("sync-push-read-only-error", {
    type: "basic",
    title: chrome.i18n.getMessage("syncStartupFailedHdg"),
    message: chrome.i18n.getMessage("syncFldrRdOnly"),
...
{
  chrome.notifications.create("native-msg-perm-error", {
    type: "basic",
    title: chrome.i18n.getMessage("syncStartupFailedHdg"),
    message: chrome.i18n.getMessage("syncPermNotif"),
    iconUrl: "img/clippings-alert.svg",
  });
}
...
{
  let message = chrome.i18n.getMessage(aMessageName);
  ...
  // `chrome.windows.create()`. If unable to get window geometry, then default
...
chrome.action.onClicked.addListener(aTab => {
  let browserAction = gLocalState.getItem("browserAction");
  if (browserAction == aeConst.BRWSACT_OPEN_CLIPPINGS_MGR) {
    ...
    // Chrome does not support sidebarAction; open the Clippings Manager window
    // as the equivalent of the Firefox sidebar.
    openClippingsManager();
  }
});


chrome.commands.onCommand.addListener(async (aCmdName, aTab) => {
...
chrome.contextMenus.onClicked.addListener(async (aInfo, aTab) => {
...
chrome.alarms.onAlarm.addListener(aAlarm => {
...
chrome.notifications.onClicked.addListener(aNotifID => {
  switch (aNotifID) {
...
chrome.storage.onChanged.addListener((aChanges, aAreaName) => {
  ...
  if (changedPrefs.includes("browserAction")) {
    gLocalState.setItem("browserAction", aChanges["browserAction"].newValue);
  }
});
...
chrome.runtime.onMessage.addListener(aRequest => {
  ...
  case "whats-new-pg-opened":
    chrome.alarms.clear("show-upgrade-notifcn");
